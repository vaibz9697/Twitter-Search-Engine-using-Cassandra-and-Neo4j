...
query2 = "create table t_by_keyword ( " \
        "location varchar," \
        "quote_count int," \
        "reply_count int," \
        ...
        "PRIMARY KEY ((keyword),like_count,tid)) WITH CLUSTERING ORDER BY(like_count DESC);"
    ...
    for key in read_data.keys():
        ...
        if keywords_processed_list is not None:
            for keyword in keywords_processed_list:
                if(not keyword==''):
                    session.execute(
                        """ INSERT INTO t_by_keyword (location,quote_count,reply_count,hashtags,datetime,date_tweet,like_count,verified,sentiment,author,tid,retweet_count,tweet_type,media_list,quoted_source_id,url_list,tweet_text,author_profile_image,author_screen_name,author_id,lang,keyword,retweet_source_id,mentions,replyto_source_id) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) """,
                                                 (location,quote_count,reply_count,hashtags,datetime,date_tweet,like_count,verified,sentiment,author,tid,retweet_count,tweet_type,media_list,quoted_source_id,url_list,tweet_text,author_profile_image,author_screen_name,author_id,lang,keyword,retweet_source_id,mentions,replyto_source_id)
                    )
                    count2 = count2 + 1
        count = count+1
    print(count,count2)