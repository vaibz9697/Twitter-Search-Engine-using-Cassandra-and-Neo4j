...
session.execute("drop table if exists t_by_hashtag2")


query8 = "create table t_by_hashtag2 ( " \
        "location varchar," \
        "quote_count int," \
        "reply_count int," \
        "hashtag varchar," \
        ...
        "mention varchar," \
        "replyto_source_id varchar," \
        "hashtagmention frozen<list<varchar>>," \
        "PRIMARY KEY ((date_tweet),hashtag,mention,tid)) WITH CLUSTERING ORDER BY(hashtag DESC);"
...
for file_name in (os.listdir(directory)):
    ...
    for key in read_data.keys():
        ...
        mentions = read['mentions']
        replyto_source_id = read['replyto_source_id']
        if hashtags is not None and mentions is not None:
            for hashtag in hashtags:
                if not hashtag=='':
                    for mention in mentions:
                        if not mention=='':
                            listtoinsert = []
                            listtoinsert.append(hashtag)
                            listtoinsert.append(mention)
                            session.execute(
                                """ INSERT INTO t_by_hashtag2 (hashtagmention,location,quote_count,reply_count,hashtag,datetime,date_tweet,like_count,verified,sentiment,author,tid,retweet_count,tweet_type,media_list,quoted_source_id,url_list,tweet_text,author_profile_image,author_screen_name,author_id,lang,keywords_processed_list,retweet_source_id,mention,replyto_source_id) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) """,
                                                         (listtoinsert,location,quote_count,reply_count,hashtag,datetime,date_tweet,like_count,verified,sentiment,author,tid,retweet_count,tweet_type,media_list,quoted_source_id,url_list,tweet_text,author_profile_image,author_screen_name,author_id,lang,keywords_processed_list,retweet_source_id,mention,replyto_source_id)
...
session.execute("drop table if exists helper2")
session.execute(
    "CREATE table helper2(date_tweet date,cnt int,hashtag varchar,mention varchar,primary key(date_tweet,cnt,hashtag,mention)) with clustering order by (cnt desc);")
output = session.execute("select date_tweet,hashtag,mention,count(tid) as cnt from t_by_hashtag2 where date_tweet='2018-1-15' group by date_tweet,hashtag,mention;")
for i in output:
    cnt = i.cnt
    mention = i.mention
    hashtag = i.hashtag
    date_tweet = i.date_tweet
    session.execute("insert into helper2(date_tweet,cnt,hashtag,mention) values ('" + str(date_tweet) + "'," + str(cnt) + ",'" +  hashtag + "','" + mention + "');" )