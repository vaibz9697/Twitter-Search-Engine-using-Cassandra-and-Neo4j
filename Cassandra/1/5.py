...
session.execute("drop table if exists t_by_popularity")


query5 = "create table t_by_popularity ( " \
        "location varchar," \
        "quote_count int," \
        "reply_count int," \
        ...
        "PRIMARY KEY ((date_tweet),like_count,tid)) WITH CLUSTERING ORDER BY(like_count DESC);"
